    ...
    <div className="flex items-center gap-3 py-2.5 px-1">
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0">
        ...
        {canRemove && (
          <button
            onClick={onRemove}
            className="p-1 rounded hover:bg-red-50 text-[var(--color-text-muted)] hover:text-[var(--color-danger)] transition-colors"
            title="Remove member"
          >
...
export function BoardMembersDialog({ boardId, boardOwnerId, onClose }: BoardMembersDialogProps) {
  ...
  const isAdmin = members.some(
    (m) => m.user_id === currentUser?.id && (m.role === 'admin' || m.user_id === boardOwnerId),
  );
  ...
  const handleSearchChange = useCallback(
    (query: string) => {
      ...
      debounceRef.current = setTimeout(async () => {
        ...
        // use members from closure — this is fine since it's a fresh callback per render
